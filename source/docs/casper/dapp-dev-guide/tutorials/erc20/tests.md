...
Let's start by defining the required constants (i.e., method names, key names, and account addresses). The following code initializes the [global state](../../../glossary/G.md#global-state) with all the data and methods that a smart contract needs to run correctly.
